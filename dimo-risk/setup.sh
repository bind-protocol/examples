...
# ── Welcome ──────────────────────────────────────────────────────────
echo ""
echo "  Bind Protocol SDK — DIMO Risk Band Example"
echo "  ───────────────────────────────────────────"
...
detected=()
for var in BIND_API_URL BIND_API_KEY CIRCUIT_ID DIMO_TOKEN_ID BIND_VERIFIER_API_KEY SHARED_PROOF_ID VERIFIER_ORG_ID; do
  if [ -n "${!var:-}" ]; then
    detected+=("$var")
  fi
done
...
# DIMO_TOKEN_ID (optional)
default_token="${DIMO_TOKEN_ID:-}"
if [ -n "$default_token" ]; then
  read -p "  DIMO_TOKEN_ID [$default_token]: " dimo_token_id
  dimo_token_id="${dimo_token_id:-$default_token}"
else
  read -p "  DIMO_TOKEN_ID [skip]: " dimo_token_id
fi
...
# ── Write .env ───────────────────────────────────────────────────────
{
  ...
  if [ -n "$circuit_id" ]; then
    ...
    echo "# CIRCUIT_ID=bind.mobility.basicriskband.v0_1_0"
  fi

  if [ -n "$dimo_token_id" ]; then
    echo ""
    echo "# DIMO vehicle token ID"
    echo "DIMO_TOKEN_ID=$dimo_token_id"
  else
    echo ""
    echo "# Optional: DIMO vehicle token ID for real data"
    echo "# DIMO_TOKEN_ID=12345"
  fi
...
echo "    npm run get-policy   Fetch the DIMO risk band policy"
